...
export function GameScene({ reducedMotion, rewardsUnlocked }: GameSceneProps) {
  ...
  useEffect(() => {
    ...
    // Three.js is loaded lazily via the curated barrel so the chunk stays
    // tree-shakeable and out of the critical bundle. Scene init is deferred to
    // an idle callback so its parse/eval can never contend with first paint.
    const startScene = () => {
      void Promise.all([
        import("../../game/three-core"),
        import("../../game/tetrominoes3d"),
      ]).then(
      ([THREE, { createTetrominoCluster, disposeScene }]) => {
        ...
        // Floating tetromino fleet — one of each kind tumbling on a wide ellipse.
        ...
        // Reward shockwave — an expanding ring fired when a reward unlocks.
        ...
        // Mouse parallax target (normalized -1..1), eased toward every frame.
    ...
    const idleId = scheduleIdle(startScene);
